...
class ContextPath(nn.Module):
    ...
    def forward(self, x):
        _, feat16, feat32 = self.resnet(x)  #resnet返回3个尺度的特征图，后面用到16x和32x的特征图
...
if __name__ == "__main__":
    ...
    #model = ContextPath().to(device)
    #summary(model,(3,512,512))
    #for i in model(x.to(device)):
    #    print(i.shape)

    x1 = torch.randn([2,256,32,32]).to(device)
    x2 = torch.randn([2,512,16,16]).to(device)
    arm16 = AttentionRefinementModule(256, 128).to(device)
    arm32 = AttentionRefinementModule(512, 128).to(device)
